...
namespace qtl
{
    namespace sqlite
    {
        ...
        {
            public:
                statement() : m_stmt(nullptr), m_fetch_result(SQLITE_OK) { }
                ...
                void open(sqlite3* db, const char* query_text, size_t text_length = -1)
                {
                    ...
                    if (tail != nullptr)
                    {
                        if (text_length == std::string::npos)
                ...
                {
                    if (value)
                        verify_error(sqlite3_bind_text(m_stmt, index + 1, value, (int)n, nullptr));
                    else
                ...
                {
                    if (value)
                        verify_error(sqlite3_bind_text16(m_stmt, index + 1, value, (int)n, nullptr));
                    else
                        verify_error(sqlite3_bind_null(m_stmt, index + 1));
                }
                ...
                void bind_param(int index, const const_blob_data& value)
                {
                    ...
                    {
                        if (value.data)
                            verify_error(sqlite3_bind_blob(m_stmt, index + 1, value.data, (int)value.size, nullptr));
                        else
                ...
                int64_t get_value_i64(int col) const
                {
                    return sqlite3_column_int64(m_stmt, col);
                }

                double get_value_f64(int col) const
                {
        ...
        class database final : public qtl::base_database<database, statement>
        {
            public:
                ...
                {
                    int result = sqlite3_open_v2(filename, &m_db, flags, nullptr);
                    if (result != SQLITE_OK)
                ...
                {
                    int result = sqlite3_exec(m_db, lpszSql, nullptr, nullptr, nullptr);
                    if (result != SQLITE_OK)
                        throw sqlite::error(result);
                }
...
#else
                    int has_moved = 0;
                    int result = sqlite3_file_control(m_db, nullptr, SQLITE_FCNTL_HAS_MOVED, &has_moved);
                    if (result != SQLITE_OK)
                        throw sqlite::error(result);
                    return has_moved == 0;
#endif //_WIN32
